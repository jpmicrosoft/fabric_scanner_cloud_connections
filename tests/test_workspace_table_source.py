...
def test_empty_table_name():
    ...
    for table_name, description in test_cases:
        try:
            ...
            scanner.read_workspaces_from_table(table_name)
...
def test_sql_injection_prevention():
    ...
    for table_name, description in injection_attempts:
        ...
        with (
            patch.object(scanner, "RUNNING_IN_FABRIC", True),
...
def test_spark_missing_column():
    ...
    with (
        ...
        if result is None:  # Should return None and fallback to API
            ...
            tests_failed += 1
            print("✗ FAIL: Should return None when workspace_id column missing")
    ...
    with (
        ...
        if result is not None and len(result) == 2:
            if result[0]["id"] == "ws-001" and result[1]["id"] == "ws-002":
                tests_passed += 1
                print("  ✓ Retrieved 2 workspaces after null filtering")
...
def test_empty_result_fallback():
    ...
    with (
        ...
        if result is None:
            tests_passed += 1
            print("  ✓ Empty table returns None")
...
def test_file_not_found_fallback():
    ...
    with (
        ...
        if result is None:
            tests_passed += 1
            print("  ✓ File not found returns None")
...
def test_personal_workspace_filtering():
    ...
    with (
        ...
        result = scanner.read_workspaces_from_table(
            "test_table", include_personal=False
        )
        ...
        if result is not None and len(result) == 2:
            tests_passed += 1
            print("  ✓ Personal workspace filtering applied")
            print("  ✓ Retrieved 2 non-personal workspaces")
