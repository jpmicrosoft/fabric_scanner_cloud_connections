    ...
    try:
        ...
        for ws in sample_workspaces:
            ...
            if capacity_id:
                ...
                shared_found += 1
                print(f"🌐 {ws_name}")
                print("   Capacity: Shared (no dedicated capacity)")
            print()

        print(
            f"Summary: {capacity_found} dedicated capacity, {shared_found} shared/personal"
        )
        print()
        ...
        batch_meta = [
            ...
            for ws in sample_workspaces[:2]
        ]  # Only process 2 to minimize API calls
        ...
        for ws_id, meta in list(sidecar.items())[:3]:  # Show first 3
            ...
            print(
                f"  is_dedicated_capacity: {meta.get('is_dedicated_capacity', False)}"
            )
            print()
        ...
        if not rows:
            ...
            print(
                f"  is_dedicated_capacity: {row_dict.get('is_dedicated_capacity', 'MISSING')}"
            )
            print()
            ...
            if missing_fields:
                ...
                print("✅ All capacity fields present in output!")
...
def test_row_creation():
    ...
    # Verify fields match
    if (
        row_dict.get("capacity_id") == test_data["capacity_id"]
        and row_dict.get("capacity_name") == test_data["capacity_name"]
        and row_dict.get("is_dedicated_capacity") == test_data["is_dedicated_capacity"]
    ):
