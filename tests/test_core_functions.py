...
def test_health_check():
    ...
    assert "quota_remaining" in mock_health_response, (
        "Should have quota_remaining field"
    )
    ...
    print("✓ PASS: Health check response structure validated")
    print(f"   API Available: {mock_health_response['api_available']}")
    print(
        f"   Quota Remaining: {mock_health_response['quota_remaining']}/{mock_health_response['quota_limit']}"
    )
...
def test_scan_result_by_id():
    ...
    print("✓ PASS: Scan result structure validated")
...
def test_incremental_scan_workspace_filtering():
    ...
    assert all(ws["modified"] >= modified_since for ws in filtered), (
        "All should be within time window"
    )

    print("✓ PASS: Incremental scan filtering logic validated")
...
def test_hash_optimization_filtering():
    ...
    mock_hash_tracker.get_workspace_hash.side_effect = [
        ...
        ),  # ws-1: unchanged, recent scan
        (
            "hash-changed",
            datetime.utcnow() - timedelta(hours=25),
        ),  # ws-2: changed hash, old scan
    ...
    # Verify
    assert len(filtered_workspaces) == 2, (
        "Should filter to 2 workspaces (ws-2 and ws-3)"
    )
    assert filtered_workspaces[0]["id"] == "ws-2", "ws-2 should be included (old scan)"
    assert filtered_workspaces[1]["id"] == "ws-3", (
        "ws-3 should be included (never scanned)"
    )

    reduction_pct = (1 - len(filtered_workspaces) / len(workspaces)) * 100
    print("✓ PASS: Hash optimization filtered workspaces correctly")
    print(f"   Original workspaces: {len(workspaces)}")
    print(f"   After optimization: {len(filtered_workspaces)}")
    print(f"   Reduction: {reduction_pct:.1f}%")
    print("   Filtered: ws-1 (scanned 2h ago, unchanged)")
    print("   Kept: ws-2 (scanned 25h ago), ws-3 (never scanned)")
        ...
        with patch("json.load", return_value=mock_json_content):
            with patch("os.path.exists", return_value=True):
                with patch("os.path.isfile", return_value=True):
                    ...
                    print("✓ PASS: JSON directory scan processed files correctly")
                    print(f"   Workspaces processed: {len(workspaces)}")
                    print(f"   Connections found: {connection_count}")
                    print("   Connection type: Sql (Azure SQL Database)")
...
def test_chunked_scan_batch_calculation():
    ...
    print("✓ PASS: Chunked scan batch calculation correct")
...
def test_workspace_batching():
    ...
    print("✓ PASS: Workspace batching created correct chunks")
...
def test_checkpoint_functionality():
    ...
    # Calculate progress
    progress_pct = (
        checkpoint_data["completed_batches"] / checkpoint_data["total_batches"]
    ) * 100
    remaining_batches = (
        checkpoint_data["total_batches"] - checkpoint_data["completed_batches"]
    )

    # Verify checkpoint structure
    assert "scan_type" in checkpoint_data, "Checkpoint should have scan_type"
    assert "completed_batches" in checkpoint_data, (
        "Checkpoint should have completed_batches"
    )
    assert checkpoint_data["completed_batches"] < checkpoint_data["total_batches"], (
        "Should have remaining work"
    )

    print("✓ PASS: Checkpoint functionality validated")
...
def test_personal_workspace_filtering():
    ...
    filtered_include = [
        ...
        if include_personal or ws["type"] != "PersonalWorkspace"
    ]
    ...
    # Verify
    assert len(filtered_include) == 5, (
        "Should include all 5 workspaces when include_personal=True"
    )
    assert len(filtered_exclude) == 3, (
        "Should include only 3 workspaces when include_personal=False"
    )

    personal_count = sum(
        1 for ws in all_workspaces if ws["type"] == "PersonalWorkspace"
    )
    team_count = sum(1 for ws in all_workspaces if ws["type"] == "Workspace")

    print("✓ PASS: Personal workspace filtering works correctly")
    print(f"   Total workspaces: {len(all_workspaces)}")
    ...
    print(
        f"   With include_personal=False: {len(filtered_exclude)} workspaces (personal excluded)"
    )
...
def test_cli_argument_parsing():
    ...
    test_cases = [
        ...
        {"args": ["--health-check"], "expected": {"health_check": True}},
    ]
    ...
    assert passed == len(test_cases), (
        f"Should pass all {len(test_cases)} CLI parsing tests"
    )

    print("✓ PASS: CLI argument parsing validated")
    print(f"   Test cases: {len(test_cases)}")
    print(f"   Passed: {passed}")
    print("   Modes tested: full-scan, incremental, scan-id, chunked, health-check")
